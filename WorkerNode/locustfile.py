...
class Mongouser(User):
    ...
    keySpace = list(os.environ['KEYSPACE'])
    keySize = int(os.environ['KEYCOUNT'])                                                # 15 ^ 5 = 759,375

    forDate = os.environ['FORDATE']
    minDate = datetime.strptime(forDate+"-01", "%Y-%m-%d")
    maxRunTime = 3*60                                                            # 15 mins change it up 
    minHour = 9
    maxHour = 17
    ...
    taskRatios = os.environ['TASKRATIOS'].split(",")

    @tag('uc_write_ts')
    @task(int(taskRatios[0]))
    def uc_write_ts(self):
        try:
            ...
            # thruput tracking
            self.environment.events.request_success.fire(request_type="pymongo", name="uc_write_ts", response_time=(time.time()-self.tic), response_length=0)
            ...
            self.environment.events.request_failure.fire(request_type="pymongo", name="uc_write_ts", response_time=(time.time()-self.tic), response_length=0)
            connect_problem = True

    @tag('uc_write_upsert')
    @task(int(taskRatios[1]))
    def uc_write_upsert(self):
        try:
            user = ''.join(self.f.random_choices(elements=self.keySpace , length=self.keySize))
            qty = random.randint(15,1015)
            forDate = self.forDate

            # thruput tracking
            self.tic = time.time()

            output = self.client.iot.ups.update_one({"user":user, "forMonth": forDate}, {"$inc": { "qty" : qty } } , upsert=True)

            # thruput tracking
            self.environment.events.request_success.fire(request_type="pymongo", name="uc_write_upsert", response_time=(time.time()-self.tic), response_length=0)
            ...
            print(f'{datetime.now()} - DB-CONNECTION-PROBLEM: ' f'{str(e)}')
            self.environment.events.request_failure.fire(request_type="pymongo", name="uc_write_upsert", response_time=(time.time()-self.tic), response_length=0)
            connect_problem = True

    @tag('uc_window_count')
    @task(int(taskRatios[2]))
    def uc_window_count(self):
        try:
            user = ''.join(self.f.random_choices(elements=self.keySpace , length=self.keySize))
            ...
            # thruput tracking
            self.environment.events.request_success.fire(request_type="pymongo", name="uc_window_count", response_time=(time.time()-self.tic), response_length=0)
            ...
            print(f'{datetime.now()} - DB-CONNECTION-PROBLEM: ' f'{str(e)}')
            self.environment.events.request_failure.fire(request_type="pymongo", name="uc_window_count", response_time=(time.time()-self.tic), response_length=0)
            connect_problem = True

    @tag('uc_window_time')
    @task(int(taskRatios[3]))
    def uc_window_time(self):
        try:
            user = ''.join(self.f.random_choices(elements=self.keySpace , length=self.keySize))
            maxDate = self.minDate + timedelta(days=30)
            whenDay = self.f.date_between_dates(self.minDate,maxDate)
            when = datetime.combine(whenDay, datetime.max.time())
            # all dates are jan so we can skip between logic and just say less than
            pipeline = [{ "$match": { "meta.name": user, "ts":{"$lt":when}} }, { "$group": { "_id": { "name": "$meta.name", "time": { "$dateTrunc": { "date": "$ts", "unit": "day", "binSize": 24 } } }, "high": { "$max": "$qty" }, "avg":{"$avg":"$qty"}, "low": { "$min": "$qty" }, "open": { "$first": "$qty" }, "close": { "$last": "$qty" } } }, { "$setWindowFields": { "partitionBy": "$_id.name", "sortBy": { "_id.time": 1 }, "output": { "avgWindow": {"$avg": "$close" } } } }]

            # thruput tracking
            self.tic = time.time()

            output = self.handle.aggregate(pipeline, hint="meta.name_1_ts_1")

            # thruput tracking
            self.environment.events.request_success.fire(request_type="pymongo", name="uc_window_time", response_time=(time.time()-self.tic), response_length=0)
            ...
            print(f'{datetime.now()} - DB-CONNECTION-PROBLEM: ' f'{str(e)}')
            self.environment.events.request_failure.fire(request_type="pymongo", name="uc_window_time", response_time=(time.time()-self.tic), response_length=0)
            connect_problem = True

    @tag('uc_findone')
    @task(int(taskRatios[4]))
    def uc_findone(self):
        try:
            user = ''.join(self.f.random_choices(elements=self.keySpace , length=self.keySize))
            forDate = self.forDate
            ...
            output = self.client.iot.ups.find_one({"user":user, "forMonth":forDate})

            # thruput tracking
            self.environment.events.request_success.fire(request_type="pymongo", name="uc_findone", response_time=(time.time()-self.tic), response_length=0)
            ...
            print(f'{datetime.now()} - DB-CONNECTION-PROBLEM: ' f'{str(e)}')
            self.environment.events.request_failure.fire(request_type="pymongo", name="uc_findone", response_time=(time.time()-self.tic), response_length=0)
            connect_problem = True